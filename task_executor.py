...
def execute_monitoring(wallet, quicknode_url, codex_api_key, alerts_config):
    ...
    while True:
        try:
            ...
            # getSignaturesForAddress only returns up to `limit` signatures per call
            # (newest-first). A single page isn't enough for a busy wallet that produced
            # more than `limit` new signatures since the last poll - the ones older than
            # the page would otherwise be silently skipped forever, since last_sig always
            # jumps straight to the newest signature seen. So keep paginating backward
            # with `before` until a page comes back short of the limit (i.e. we've reached
            # last_sig / the start of this wallet's history) to guarantee nothing in
            # between is missed.
            page_limit = 50
            max_pages = 20  # hard cap (up to 1000 sigs/poll) so a long gap since the last
            # poll can't walk a bot wallet's entire multi-thousand-tx history in one go
            sig_infos = []

            if last_sig is None:
                # First-ever poll for this wallet: no floor to catch up to yet, so just take
                # one small page as the starting baseline (matches prior behavior) rather than
                # paginating back through history and alerting on a backlog of old trades.
                sig_infos = rpc_call(quicknode_url, "getSignaturesForAddress", [wallet_address, {"limit": 10}]) or []
            else:
                before_cursor = None
                for _ in range(max_pages):
                    params = {"limit": page_limit, "until": last_sig}
                    if before_cursor:
                        params["before"] = before_cursor

                    page = rpc_call(quicknode_url, "getSignaturesForAddress", [wallet_address, params])
                    if not page:
                        break

                    sig_infos.extend(page)
                    if len(page) < page_limit:
                        break  # short page means we've caught up to last_sig

                    before_cursor = page[-1]["signature"]
                else:
                    click.echo(
                        f"[SolTracker] {wallet_name} ({wallet_address}): hit the {max_pages}-page "
                        f"cap while paginating ({len(sig_infos)} sig(s) fetched) - wallet may be "
                        f"falling further behind than one poll can catch up on"
                    )
            ...
            if sig_infos:
                ...
                for info in sig_infos:
                    if info.get("err") is not None:
                        outcome_counts["onchain_error"] = outcome_counts.get("onchain_error", 0) + 1
                    ...
                    try:
                        ...
                        if tx_result:
                            outcome = process_transaction(
                                wallet, tx_result, info["signature"], codex_api_key, alerts_config
                            )
                        else:
                            outcome = "empty_tx_result"

                        if outcome in ("buy", "sell"):
                            click.echo(
                                f"[SolTracker] {wallet_name} - {info['signature']}: detected "
                                f"{'BUY' if outcome == 'buy' else 'SELL'}, sending alert"
                            )
...
def process_transaction(wallet, tx_result, signature, codex_api_key, alerts_config):
    ...
    aggregate into a per-poll summary) rather than logging directly - logging
    once per transaction was found to be unreliable under bursty load (a
    single poll for a busy wallet can process 10+ transactions almost
    instantly, and that many log lines landing at once risks being dropped
    by the hosting platform's log-rate limiting).
    """
    wallet_address = wallet["address"]
    wallet_name = wallet["name"]
    ...
    if meta.get("err") is not None:
        return "onchain_error"  # failed transaction, nothing actually settled
